...
<header>
  <div class="title-block">
    ...
    <div>
      ...
      <span class="sub" id="hdr-sub">6 courses · 118 nodes · 1,121 connections</span>
    </div>
  </div>
  <div class="controls">
  ...
  <section id="detail">
    ...
    <div id="dashboard">
      ...
      <div>
        ...
        <div class="stat-grid">
          <div class="stat-card"><div class="n" data-count="Course">6</div><div class="label">Courses</div></div>
          <div class="stat-card"><div class="n" data-count="Session">74</div><div class="label">Sessions</div></div>
          <div class="stat-card"><div class="n" data-count="Concept">23</div><div class="label">Concepts</div></div>
          <div class="stat-card"><div class="n" data-count="Assignment">11</div><div class="label">Assignments</div></div>
        </div>
      </div>
      <div>
        ...
        <div class="legend-item" data-filter="Course">
          ...
          Course <span class="legend-cnt"><span data-cnt="Course">6</span> · click to expand</span>
        </div>
        <div class="legend-item" data-filter="Session">
          <span class="legend-dot" style="background:#b8864e"></span>
          Session <span class="legend-cnt"><span data-cnt="Session">74</span> · hidden in overview</span>
        </div>
        <div class="legend-item" data-filter="Concept">
          <span class="legend-dot diamond" style="background:#2a6b42"></span>
          Concept <span class="legend-cnt"><span data-cnt="Concept">23</span> · always visible</span>
        </div>
        <div class="legend-item" data-filter="Reference">
          <span class="legend-dot square" style="background:#87867f"></span>
          Reference <span class="legend-cnt"><span data-cnt="Reference">4</span></span>
        </div>
        <div class="legend-item" data-filter="Assignment">
          <span class="legend-dot" style="background:#9a6520"></span>
          Assignment <span class="legend-cnt"><span data-cnt="Assignment">11</span> · always visible</span>
        </div>
      </div>
      <div>
...
const styles = [
  ...
  },
  {
    ...
    style: {
      ...
      'curve-style': 'bezier',
      opacity: 0.22,            /* visible-but-quiet at rest; focus mode lights the active neighbourhood */
      'arrow-scale': 0.6,
  ...
  /* ── Focus mode: dim the field, light up the active neighbourhood ── */
  {
    selector: 'node.dim',
    style: { opacity: 0.18, 'transition-property': 'opacity', 'transition-duration': '160ms', 'transition-timing-function': 'ease-out' }
  },
  {
    selector: 'edge.dim',
    style: { opacity: 0.03 }
  },
  {
    selector: 'edge.lit',
    style: {
      opacity: 0.92, width: 2.2,
      'line-color': '#c96442', 'target-arrow-color': '#c96442',
      'arrow-scale': 0.75, 'z-index': 900,
    }
  },
  {
    selector: 'node.lit',
...
// ── Init Cytoscape ────────────────────────────────────────────
// fcose = far better force layout (less overlap); fall back to built-in cose if the CDN missed.
...
cy.on('tap', 'node[type = "Course"]', evt => {
  ...
  if (expanded.has(id)) {
    ...
    expandCourse(id);
    const course = cy.getElementById(id);
    cy.animate({ fit: { eles: course.closedNeighborhood(':visible'), padding: 70 }, duration: 450, easing: 'ease-out-quint' });
    focusNode(course);
    setTimeout(() => drawMM(), 520);
  }
});

// ── Expand / Collapse all buttons ─────────────────────────────
document.getElementById('btn-expand').addEventListener('click', () => {
  cy.nodes('[type = "Session"]').style('display', 'element');
  cy.nodes('[type = "Course"]').forEach(c => expanded.add(c.id()));
  clearFocus();
  runOverviewLayout({ fit: true });                 // proper global layout of all 118 nodes
  updateCtxBar();
  setTimeout(() => { cy.fit(cy.elements(':visible'), 40); drawMM(); }, 200);
});
...
// ── Overview layout (visible nodes only) ─────────────────────
function runOverviewLayout(opts) {
  opts = opts || {};
  const fit = opts.fit !== false, animate = !!opts.animate;
  const base = { animate, animationDuration: 500, fit, padding: 60, nodeDimensionsIncludeLabels: true };
  // fcose needs the EDGES in the collection (node-only degenerates ~23x tighter). Pass elements.
  const eles = cy.elements(':visible');
  const fcoseCfg = Object.assign({ name: 'fcose', quality: 'proof', randomize: true, packComponents: true,
    nodeSeparation: 130, idealEdgeLength: 95, nodeRepulsion: 9000, edgeElasticity: 0.45,
    gravity: 0.2, gravityRange: 3.8, numIter: 2500 }, base);
  const coseCfg = Object.assign({ name: 'cose', nodeRepulsion: () => 14000, nodeOverlap: 24,
    idealEdgeLength: () => 85, edgeElasticity: () => 50, gravity: 0.7, numIter: 900, randomize: false }, base);
  // Select by trying fcose and catching — cytoscape throws synchronously on an unregistered layout.
  try { eles.layout(fcoseCfg).run(); }
  catch (e) { try { eles.layout(coseCfg).run(); } catch (e2) {} }
}
function applyCounts() {
  const B = window.BUNDLE; if (!B || !B.nodes) return;
  const c = {}; B.nodes.forEach(n => { const t = n.data.type; c[t] = (c[t] || 0) + 1; });
  const nodes = B.nodes.length, edges = (B.edges || []).length;
  const sub = document.getElementById('hdr-sub');
  if (sub) sub.textContent = (c.Course||0) + ' courses · ' + nodes + ' nodes · ' + edges.toLocaleString() + ' connections';
  document.querySelectorAll('[data-count]').forEach(el => { el.textContent = c[el.getAttribute('data-count')] || 0; });
  document.querySelectorAll('[data-cnt]').forEach(el => { el.textContent = c[el.getAttribute('data-cnt')] || 0; });
}
applyCounts();
...
// ── Node click ────────────────────────────────────────────────
// ── Focus mode: light the active neighbourhood, dim the rest ───
function focusNode(n) {
  const hood = n.closedNeighborhood();          // n + neighbours + connecting edges
  cy.elements().addClass('dim');
  hood.removeClass('dim');
  hood.edges().addClass('lit');
  hood.nodes().addClass('lit');
}
function clearFocus() { cy.elements().removeClass('dim lit'); }

cy.on('tap', 'node', evt => { focusNode(evt.target); showDetail(evt.target.id()); });
cy.on('tap', evt => { if (evt.target === cy) { clearFocus(); showDash(); } });
cy.on('mouseover', 'node', evt => { if (!cy.$('node:selected').length) focusNode(evt.target); });
cy.on('mouseout', 'node', () => { const s = cy.$('node:selected'); if (s.length) focusNode(s[0]); else clearFocus(); });
